...
// Doer Management — MD only, a PC doesn't get this either.
router.get("/", listsController.list);
router.post("/", requireRole("MD"), validate({ body: createListSchema }), listsController.create);
router.patch(
  "/:id/members",
  requireRole("MD"),
...
router.delete("/:id", requireRole("MD"), validate({ params: idParamSchema }), listsController.remove);
