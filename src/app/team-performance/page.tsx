...
function TeamPerformanceInner() {
  ...
  async function handleArchiveSubmit() {
    ...
    try {
      ...
      alert(`Weekly Performance Summary for ${weekLabel} archived successfully!`);
  ...
  function exportCSV() {
    ...
    const rows = (dgmaxSummary?.summaries ?? []).map((s) => [
      dgmaxSummary?.weekLabel || weekLabel,
      s.doerName,
      s.department,
      s.assignedTasks,
      s.completedTasks,
      ...
      <div className="md:ml-64 flex-1 flex flex-col bg-background min-h-screen">
        ...
        <header className="hidden md:flex bg-surface w-full border-b-2 border-on-surface justify-between items-center h-16 px-container-padding sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={exportCSV}
              className="px-4 py-1.5 border-2 border-on-surface bg-surface font-label-sm text-xs uppercase hover:bg-surface-container transition-colors cursor-pointer font-bold"
            >
              Export CSV
            </button>
            {user?.role === "Admin" && (
              <button
                ...
                className="px-4 py-1.5 bg-on-surface text-surface-container-lowest border-2 border-on-surface font-label-sm text-xs uppercase hover:bg-primary transition-colors cursor-pointer font-bold"
              >
                Archive Week
              </button>
            )}
        ...
        <main className="flex-1 p-4 md:p-stack-lg flex flex-col gap-stack-lg max-w-full overflow-hidden">
          {/* Tabs: Current Week vs Archive History */}
          <div className="flex items-center justify-between border-b-2 border-on-surface pb-2">
            <div className="flex gap-2">
              <button
                onClick={() => setActiveTab("current")}
                className={`px-4 py-2 border-2 border-on-surface font-label-sm text-xs uppercase font-bold transition-colors cursor-pointer ${
                  activeTab === "current"
                    ? "bg-on-surface text-surface"
                    : "text-on-surface hover:bg-surface-container"
                }`}
              >
                Current Week Review ({dgmaxSummary?.weekLabel || weekLabel})
              </button>
              <button
              ...
              <div className="bg-surface-container-lowest border-2 border-on-surface overflow-x-auto max-w-full">
                <table className="w-full text-left border-collapse min-w-[720px]">
                  ...
                  <tbody className="font-body-md text-sm">
                    {loading && (
                      <tr><td colSpan={7} className="py-6 text-center font-data-mono">Loading weekly summary...</td></tr>
                    )}
                    {!loading && (dgmaxSummary?.summaries.length ?? 0) === 0 && (
                      <tr><td colSpan={7} className="py-6 text-center font-data-mono">No active doers found.</td></tr>
                    )}
            ...
            <div className="flex flex-col gap-6">
              {archivedWeeks.length === 0 ? (
                <div className="bg-surface border-2 border-on-surface p-8 text-center font-data-mono text-on-surface-variant">
                  No archived weeks yet. Click &quot;Archive Week&quot; to lock in weekly performance reviews.
                </div>
              ) : (
                  ...
                  <div key={week} className="bg-surface border-2 border-on-surface p-4 flex flex-col gap-3">
                    <div className="flex items-center justify-between border-b border-on-surface pb-2">
                      ...
                      <span className="font-data-mono text-xs text-on-surface-variant">
                        Archived On: {formatDMY(items[0]?.archivedAt?.slice(0, 10))}
                      </span>
                    </div>
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        <thead className="bg-surface-container font-label-sm uppercase border-b border-on-surface">
                          <tr>
                            ...
                            <th className="p-2 border-r border-surface-variant text-center text-emerald-800 font-bold">
                              🟢 Green
                            </th>
                            <th className="p-2 border-r border-surface-variant text-center text-amber-800 font-bold">
                              🟡 Yellow
                            </th>
                            <th className="p-2 border-r border-surface-variant text-center text-rose-800 font-bold">
                              🔴 Red
                            </th>
                        ...
                        <tbody>
                          {items.map((row) => (
                            <tr key={row.id} className="border-b border-surface-variant">
                              ...
                              <td className="p-2 border-r border-surface-variant text-center font-data-mono">
                                {row.assignedTasks}
                              </td>
                              <td className="p-2 border-r border-surface-variant text-center font-data-mono font-bold text-emerald-700">
                                {row.greenCount}
                              </td>
                              <td className="p-2 border-r border-surface-variant text-center font-data-mono font-bold text-amber-700">
                                {row.yellowCount}
                              </td>
                              <td className="p-2 border-r border-surface-variant text-center font-data-mono font-bold text-rose-700">
                                {row.redCount}
                              </td>
                              <td className="p-2 border-r border-surface-variant text-center font-data-mono">
                                {row.pendingCount}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl bg-surface-container-lowest border-2 border-on-surface flex flex-col">
            <div className="flex items-center justify-between border-b-2 border-on-surface p-3 bg-surface">
              ...
              <button onClick={() => setShowArchiveModal(false)} className="font-label-sm text-xs uppercase hover:underline cursor-pointer">
                Close
              </button>
            </div>
            ...
            <div className="p-4 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  className="border-2 border-on-surface bg-surface px-3 py-1 font-data-mono text-sm text-on-surface focus:outline-none"
                />
              </div>

              <div className="flex flex-col gap-3">
                <h4 className="font-label-sm text-xs uppercase font-bold border-b border-on-surface pb-1">
                  Manager Remarks per Employee:
                </h4>
                {dgmaxSummary?.summaries.map((s) => (
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-on-surface">
                <button
                  ...
                  className="px-4 py-2 border-2 border-on-surface font-label-sm text-xs uppercase hover:bg-surface-container transition-colors cursor-pointer font-bold"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-on-surface text-surface-container-lowest border-2 border-on-surface font-label-sm text-xs uppercase hover:bg-primary transition-colors cursor-pointer font-bold disabled:opacity-50"
                >
